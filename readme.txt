...
= 1.3.0 =
* 🔒 安全修复：IDOR漏洞 — 对话历史添加用户隔离
* 🔒 安全修复：存储型XSS — AI标签经过strip_tags过滤
* 🐛 修复：许可证管理后台添加登录保护
* 🐛 修复：对话历史菜单项缺失及Nonce验证冲突
* 🐛 修复：消息显示乱码、会话ID空值保护
* 🐛 修复：访客记录显示、前台聊天区块session_id补全
* 🐛 修复：SEO批量优化模型参数名错误
* 🐛 修复：flushPostCache无缓存开关、replaceMode空内容问题
* 🌟 新增：对话历史30天自动清理
* 🌟 优化：许可证5分钟本地缓存、WP兼容性兜底
