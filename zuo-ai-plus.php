    ...
    add_filter('template_include', function ($template) {
        ...
        if ($isNavTemplate) {
            add_action('wp_enqueue_scripts', function () {
                ...
                wp_enqueue_script('chart-js', AI_PLUS_PLUGIN_URL . 'Assets/vendor/chart.umd.min.js', [], '4.4.7', true);
                wp_enqueue_script('zuo-nav-js', AI_PLUS_PLUGIN_URL . 'Assets/js/nav.js', ['chart-js'], AI_PLUS_VERSION, true);
                ...
                if (is_singular('nav_site')) {
                    ...
                    $navConfig['viewStatsUrl'] = esc_url(rest_url('ai-plus/v1/nav/views?post_id=' . $postId));
                }
...
register_activation_hook(__FILE__, function () {
    ...
    // 设置导航日志清理任务（每天清理90天前的日志）
    if (!wp_next_scheduled('ai_plus_nav_log_cleanup')) {
        wp_schedule_event(time(), 'daily', 'ai_plus_nav_log_cleanup');
    }
...
// 导航日志清理回调
add_action('ai_plus_nav_log_cleanup', [\ZuoAIPlus\Controllers\NavigationController::class, 'cleanupOldLogs']);
...
add_action('enqueue_block_editor_assets', function () {
    wp_enqueue_script(
        ...
        ['wp-blocks', 'wp-editor', 'wp-element', 'wp-components', 'wp-compose', 'wp-api', 'wp-api-fetch'],
        AI_PLUS_VERSION,
        true
    );
...
add_filter('the_content', function ($content) {
    ...
    foreach ($blocks as $block) {
        ...
        $chat_html .= '<div class="ai-plus-chat-rendered" data-model="' . $model . '" >';
        // 用隐藏 input 而非 data 属性:避免 HTML 转义破坏文章内容
        $chat_html .= '<input type="hidden" class="ai-article-context-val" value="' . esc_attr($articleText) . '">';
        $chat_html .= '<div class="ai-plus-chat-title">' . $title . '</div>';
        $chat_html .= '<div class="ai-plus-chat-messages">';

        if (empty($messages)) {
            $chat_html .= '<p class="ai-plus-chat-empty">开始对话吧...</p>';
        } else {
            foreach ($messages as $m) {
                $role   = ($m['role'] ?? '') === 'user' ? 'user' : 'assistant';
                $text   = esc_html($m['content'] ?? '');
                $chat_html .= '<div class="ai-plus-msg ' . $role . '">' . nl2br($text) . '</div>';
            }
        }

        $chat_html .= '</div>';
        $chat_html .= '<div class="ai-plus-chat-bar">';
        $chat_html .= '<textarea class="ai-plus-chat-input" rows="2" placeholder="输入消息... (Enter发送,Shift+Enter换行)" ></textarea>';
        $chat_html .= '<button class="ai-plus-chat-send" >发送</button>';
        $chat_html .= '<span class="ai-plus-chat-loading" >⏳</span>';
        $chat_html .= '</div></div>';
    }
