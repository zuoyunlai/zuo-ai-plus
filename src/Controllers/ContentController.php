...
class ContentController extends BaseController
{
    ...
    private function buildPrompt(string $action, string $content, string $extraPrompt): string
    {
        ...
        return match ($action) {
            // SEO 友好的文章生成 prompt
            'generate' => "你是一位有多年写作经验的中文博主,擅长用自然、接地气的语言写实用文章。请根据标题撰写一篇高质量文章。\n\n【内容要求】\n- 字数800字以上,内容充实有干货\n- 像给朋友分享经验一样写,不是写百科词条\n- 用口语化但不随意的风格,偶尔用「我们」「你」这样的称呼拉近距离\n- 穿插真实场景、具体例子、个人观点,不要全是定义和概念\n- 观点要有立场,不要端水式'各有优劣'的废话\n- 适当加入过渡句和承上启下,让段落之间自然衔接,不像拼接段落\n\n【排版要求】\n- 用 ## 和 ### 小标题分节(不要写一级标题#),小标题要有吸引力,像公众号文章的段落标题\n- 适合展示数据时用 Markdown 表格:| 列1 | 列2 | 然后 |---|---| 再写数据行\n- 列举要点、步骤时用有序列表(1. 2. 3.)或无序列表(- ),但不要整篇只有列表\n- 适合配图处插入 ![图片描述](image_placeholder)\n\n【严格禁止——这些是AI写作的典型痕迹】\n- 禁止'在当今...的时代'、'随着...的发展'这类开头\n- 禁止'综上所述'、'总而言之'、'总的来说'这类总结套话\n- 禁止'不仅...而且'、'既...又...'的排比堆砌\n- 禁止每段都用'首先...其次...最后...'的八股结构\n- 禁止空洞的总结段(如'希望本文对你有所帮助')\n- 禁止输出任何思考过程、英文说明或内部提示\n\n{$kbBlock}{$extra}\n文章标题:{$content}",

            'rewrite' => "你是一位有多年写作经验的中文博主。请将以下文章用更自然、更有人味的方式改写。{$kbBlock}{$extra}\n【改写要求】\n- 保持原意和核心信息不变,不要遗漏要点\n- 用口语化、接地气的风格重写,像在给朋友讲解,不要像教科书\n- 句式要多样,长短句交替,不要全是长句堆砌\n- 适当加入过渡句,让段落衔接自然\n- 可以补充具体例子或场景来增强说服力\n- 直接输出改写结果,不要任何前缀、编号或解释\n\n【禁止AI痕迹】\n- 禁止「综上所述」「总而言之」等套话\n- 禁止「不仅…而且」「既…又…」排比堆砌\n- 禁止「首先…其次…最后」八股结构\n- 禁止空洞结尾(如「希望对你有所帮助」)\n\n原文:\n{$content}",

            'expand'  => "你是一位有多年写作经验的中文博主。请在原文基础上自然地续写内容。{$kbBlock}{$extra}\n【续写要求】\n- 不重复已有内容,从原文结尾处自然衔接\n- 补充更多具体案例、细节、实操经验,让内容更有干货\n- 保持与原文一致的口语化风格,像同一个人写的\n- 适当用 ## 小标题分节,让续写部分结构清晰\n- 直接续写,不要重复标题、不要编号\n\n【禁止AI痕迹】\n- 禁止「综上所述」「总而言之」等套话\n- 禁止「不仅…而且」「既…又…」排比堆砌\n- 禁止空洞结尾\n\n原文:\n{$content}",
